...
Options (after the `--`):
  ...
  --fog-density D        ground fog peak density per metre (default 4.5e-3)
...
def parse_args():
    ...
    p.add_argument("--fog-density", type=float, default=4.5e-3)
    p.add_argument("--no-fog", action="store_true")
    p.add_argument("--no-haze", action="store_true")
    p.add_argument("--haze-density", type=float, default=1.3e-4)
